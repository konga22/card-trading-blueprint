  ...
  CATALOG_GAME_KEYS.map((key) => ({
    ...
    description:
      key === "all"
        ...
        : `${CATALOG_GAME_LABELS[key]} 카드만 모아서 빠르게 좁혀 봅니다.`,
  }));
...
export const CATALOG_PURPOSE_FILTERS: readonly CatalogFilterOption<CatalogPurposeKey>[] =
  CATALOG_PURPOSE_KEYS.map((key) => ({
    ...
    description:
      key === "beginner"
        ? "처음 거래를 시작하는 사용자가 보기 쉬운 카드와 흐름을 모았습니다."
        : `${CATALOG_PURPOSE_LABELS[key]} 목적에 맞는 카드만 다시 모아 봅니다.`,
  }));
...
export const CATALOG_FORMAT_FILTERS: readonly CatalogFilterOption<CatalogFormatKey>[] =
  CATALOG_FORMAT_KEYS.map((key) => ({
    ...
    description: `${CATALOG_FORMAT_LABELS[key]} 기준으로 결과를 좁힙니다.`,
  }));

export const CATALOG_DETAIL_FILTERS = [
  { key: "set", label: "세트", description: "확장팩과 세트명으로 결과를 나눕니다." },
  { key: "cardNumber", label: "카드번호", description: "정확한 카드번호로 바로 특정 카드를 찾습니다." },
  { key: "language", label: "언어", description: "한국어, 영어, 일본어 기준으로 결과를 걸러냅니다." },
  { key: "rarity", label: "레어도", description: "희소 등급 기준으로 다시 비교합니다." },
  { key: "edition", label: "에디션", description: "초판, 재판, 프로모 여부를 분리합니다." },
  { key: "holoType", label: "홀로 타입", description: "표면 처리와 홀로 타입으로 세부 분류합니다." },
  { key: "condition", label: "상태", description: "민트급, 사용감 있음, 그레이딩처럼 상태를 좁힙니다." },
  { key: "gradingCompany", label: "감정사", description: "PSA, BGS 같은 감정사 기준으로 비교합니다." },
  { key: "gradeScore", label: "등급 점수", description: "그레이딩 점수 범위로 결과를 더 좁힙니다." },
] as const;
